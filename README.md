...
- 从“搜索地点”升级为“生成可执行行程”：系统不是简单列出 POI，而是把自然语言需求转成时间顺序明确、场所可定位、路线可查看的完整方案。
- 隐性约束主动推断：根据“带孩子”“减肥”“别太远”“朋友聚会”等语义，自动补全亲子友好、低卡餐、儿童座椅、通勤半径、包厢需求等用户没明说的条件。
- LLM 与规则混合规划：规则层保证稳定可跑，LLM 用来增强模糊意图和自然表达，避免纯模型方案不可控，也避免纯规则体验僵硬。
- 工具链闭环：规划结果继续连接高德路线、POI、餐厅预约、票务、额外下单和行程卡，形成“理解 → 推荐 → 执行 → 复盘”的闭环。
- 面向真实部署的密钥边界：LLM key 和高德 Web Service key 始终留在 FastAPI 或 Cloudflare Functions，前端只拿公开配置，适合从本地 demo 平滑迁移到线上。
- 游客与注册用户双路径：游客可零门槛体验核心规划，注册用户保留历史行程，降低首次使用阻力，也保留长期使用价值。
